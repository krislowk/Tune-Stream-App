...
function discoverAndInit() {
    ...
    if (!nFuncName) {
        try {
            var testInput = "vynce_test_12345";
            ...
            var skipList = ["setTimeout", "setInterval", "clearTimeout", "clearInterval", "eval", "prompt", "confirm", "alert", "print", "stop", "open", "close", "focus", "blur", "postMessage", "captureEvents", "releaseEvents", "getComputedStyle", "matchMedia", "moveTo", "moveBy", "resizeTo", "resizeBy", "scroll", "scrollTo", "scrollBy", "atob", "btoa", "fetch", "createImageBitmap"];

            for (var i = 0; i < keys.length; i++) {
                try {
                    var key = keys[i];
                    // Skip known non-candidates
                    if (key.startsWith("webkit") || key.startsWith("on") || key.startsWith("crypto") ||
                        key === "CipherBridge" || key === "_cipherSigFunc" ||
                        key === "_nTransformFunc" || key === "window" || key === "self" || key === "document" || key === "location" ||
                        skipList.indexOf(key) !== -1) {
                        skipped++;
                        continue;
                    }
