...
class PoTokenWebView private constructor(
    ...
    init {
        ...
        webView.webChromeClient = object : WebChromeClient() {
            override fun onConsoleMessage(m: ConsoleMessage): Boolean {
                ...
                when (m.messageLevel()) {
                    ConsoleMessage.MessageLevel.ERROR -> println("JS: $msg")
                    ConsoleMessage.MessageLevel.WARNING -> println("JS: $msg")
                    else -> println("JS: $msg")
                }
                ...
                if (msg.contains("Uncaught")) {
                    ...
                    println("This WebView implementation is broken: $fmt")
    ...
    private fun loadHtmlAndObtainBotguard() {
        println("loadHtmlAndObtainBotguard() called")
    ...
    fun downloadAndRunBotguard() {
        println("downloadAndRunBotguard() called")
    ...
    fun onRunBotguardResult(botguardResponse: String) {
        println("botguardResponse: $botguardResponse")
        makeBotguardServiceRequest(
            "https://www.youtube.com/api/jnn/v1/GenerateIT",
            "[ \"$REQUEST_KEY\", \"$botguardResponse\" ]",
        ) { responseBody ->
            println("GenerateIT response: $responseBody")
            try {
                val (integrityToken, expirationTimeInSeconds) = parseIntegrityTokenData(responseBody)
                println("Parsed integrityToken (${integrityToken.take(50)}...), expires in $expirationTimeInSeconds sec")
                ...
                println("Evaluating createPoTokenMinter JavaScript...")
                webView.evaluateJavascript(
                    """try {
                ...
                println("Failed to parse integrity token data: ${e.message}")
    ...
    @JavascriptInterface
    fun onMinterCreated() {
        println("poToken minter created successfully, initialization complete")
    ...
    suspend fun generatePoToken(identifier: String): String {
        return withContext(Dispatchers.Main) {
            suspendCancellableCoroutine { cont ->
                println("generatePoToken() called with identifier $identifier")
    ...
    fun onObtainPoTokenResult(identifier: String, poTokenU8: String) {
        println("Generated poToken (before decoding): identifier=$identifier poTokenU8=$poTokenU8")
        val poToken = try {
        ...
        println("Generated poToken: identifier=$identifier poToken=$poToken")
