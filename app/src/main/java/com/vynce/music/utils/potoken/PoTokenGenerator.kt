package com.vynce.music.utils.potoken

import android.webkit.CookieManager
import com.vynce.music.utils.potoken.BadWebViewException
...
class PoTokenGenerator {
    ...
    fun getWebClientPoToken(videoId: String, sessionId: String): PoTokenResult? {
        println("getWebClientPoToken called: videoId=$videoId, sessionId=$sessionId")
        println("WebView state: supported=$webViewSupported, badImpl=$webViewBadImpl")
        if (!webViewSupported || webViewBadImpl) {
            println("WebView not available: supported=$webViewSupported, badImpl=$webViewBadImpl")
            return null
        }

        return try {
            println("Calling runBlocking to generate poToken (timeout=${POTOKEN_TIMEOUT_MS}ms)...")
            runBlocking {
                withTimeout(POTOKEN_TIMEOUT_MS.milliseconds) {
            ...
            println("poToken generation timed out after ${POTOKEN_TIMEOUT_MS}ms; proceeding without PoToken: $e")
            runBlocking {
                webPoTokenGenLock.withLock {
                    try {
                        ...
                        println("Exception closing PoTokenWebView during timeout cleanup: $closeEx")
                    }
            ...
            println("poToken generation exception: ${e.javaClass.simpleName}: ${e.message}")
            when (e) {
                is BadWebViewException -> {
                    println("Could not obtain poToken because WebView is broken")
                    webViewBadImpl = true
                    null
                }
    ...
    private suspend fun getWebClientPoToken(videoId: String, sessionId: String, forceRecreate: Boolean): PoTokenResult {
        println("Web poToken requested: videoId=$videoId, sessionId=$sessionId")
        ...
        val (poTokenGenerator, streamingPot, hasBeenRecreated) =
            webPoTokenGenLock.withLock {
                ...
                if (shouldRecreate) {
                    println("Creating new PoTokenWebView (forceRecreate=$forceRecreate)")
                    ...
                    println("Streaming poToken generated for sessionId=${webPoTokenSessionId?.take(20)}...")
                }
        ...
        val playerPot = try {
            ...
            if (hasBeenRecreated) {
                ...
                // content is lost
                println("Failed to obtain poToken, retrying: $throwable")
                return getWebClientPoToken(videoId = videoId, sessionId = sessionId, forceRecreate = true)
            }
        }

        println("poToken generated successfully: player=${playerPot.take(20)}..., streaming=${streamingPot.take(20)}...")
