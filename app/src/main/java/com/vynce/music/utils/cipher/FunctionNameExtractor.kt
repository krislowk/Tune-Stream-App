...
object FunctionNameExtractor {
    ...
    private val SIG_FUNCTION_PATTERNS = listOf(
        ...
        Regex("""&&\s*\(\s*[a-zA-Z0-9$]+\s*=\s*([a-zA-Z0-9$]+)\s*\(\s*(\d+)\s*,\s*decodeURIComponent\s*\(\s*[^)]+\s*\)\s*\)"""),
        // Pattern 1a (April 2026): &&(z=hJ(6,decodeURIComponent(h.s))
        Regex("""&&\s*\(\s*[a-zA-Z0-9$]+\s*=\s*([a-zA-Z0-9$]+)\s*\(\s*(\d+)\s*,\s*decodeURIComponent\s*\(\s*[a-zA-Z0-9$]+\s*\.\s*[a-zA-Z0-9$]+\s*\)\s*\)"""),
        // Pattern 1b (May 2026): .set("signature", FUNC(NUM, decodeURIComponent(VAR)))
        Regex("""\.set\("signature"\s*,\s*([a-zA-Z0-9$]+)\s*\(\s*(\d+)\s*,\s*decodeURIComponent\s*\("""),
        // Pattern 1c (Generic May 2026): a.set("signature", b(a.s))
        Regex("""\.set\("signature"\s*,\s*([a-zA-Z0-9$]+)\s*\([^,]+\.\s*[a-zA-Z0-9$]+\s*\)"""),
        ...
        // Desperate pattern - looks for common decipher function structure
        Regex("""([a-zA-Z0-9$]+)\s*=\s*function\([a-z]\)\s*\{\s*var\s+[a-z]\s*=\s*[a-z]\.split\(""\)"""),
        Regex("""([a-zA-Z0-9$]+)\s*=\s*function\([a-z]\)\s*\{[^}]*reverse\(\)"""),
    )
    ...
    private val N_FUNCTION_PATTERNS = listOf(
        ...
        // Pattern 2a (May 2026): .set("n", FUNC(VAR))
        Regex("""\.set\("n"\s*,\s*([a-zA-Z0-9$]+)\s*\("""),
        // Pattern 2b (Generic): .get("n") && (a=b(a))
        Regex("""\.get\("n"\)\s*&&\s*\(([a-zA-Z0-9$]+)\s*=\s*([a-zA-Z0-9$]+)\(\1\)\)"""),
        // Pattern 2c (Generic alternate): .get("n") && (b = FUNC(b))
        Regex("""\.get\("n"\)\s*&&\s*([a-zA-Z0-9$]+)\s*=\s*([a-zA-Z0-9$]+)\(\1\)"""),
